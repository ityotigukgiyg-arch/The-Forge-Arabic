--[[
    🔧 الأدوات المشتركة
    📦 تستخدم في كل مهمة (Quest)
    
    الاستخدام: يتم تحميلها تلقائيًا بواسطة Loader.lua
    الوصول: _G.Shared.functionName()
--]]
...
----------------------------------------------------------------
-- 🚀 حركة سلسة (BodyVelocity + BodyGyro)
----------------------------------------------------------------
function Shared.smoothMoveTo(targetPos, stopDistance, moveSpeed, callback)
    ...
    State.moveConn = Services.RunService.Heartbeat:Connect(function()
        ...
        if distance < stopDistance then
            print(string.format("   ✅ تم الوصول إلى الهدف! (%.1f وحدات)", distance))
...
function Shared.startAntiAfk(interval, clickCount)
    ...
    task.spawn(function()
        print("🛡️ [مضاد الخمول] بدأ! النقر كل " .. interval .. " ثانية.")
        while true do
            task.wait(interval)
            pcall(function()
...
-- ✅ تم التهيئة بنجاح
----------------------------------------------------------------
print("✅ تم تحميل Shared.lua بنجاح!")
print("   📦 متاح: _G.Shared")
